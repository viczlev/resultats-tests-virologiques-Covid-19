...
# Table of contents   
0) [0) Introduction - Description de la base de données](#0)   
    A. [Sub paragraph](#0.A)   
    B. [Sub paragraph](#0.B)   
    C. [Sub paragraph](#0.C)   
1) [1) Qui a créé ce jeu de données ? A quelles fins ?](#1)   
    A. [A. Producteur du jeu de données : Santé Publique France](#1.A)   
    B. [B. A quelles fins le jeu de données est-il créé ? Mise en contexte (crise sanitaire)](#1.B)   
    C. [C. Chaîne de valeur du jeu de données : depuis la collecte de la data jusqu'à la publication des indicateurs](#1.C)   
        i) [i) Collecte de données auprès des professionnels de santé](#1.C.i)   
	ii) [ii) Centralisation par SI-DEP](#1.c.ii)   
	iii) [iii) Traitement des données anonymisées par Santé publique France](#1.c.iii)   
	iv) [iv) Publications](#1.c.iv)   
2) [2) Quelles catégories sont utilisées pour le représenter ?](#2)   
3) [3) Quels sont les usages de ce jeu de données ?](#3)  
4) [4) Quelles sont les limites ou les critiques rencontrées par ce jeu de données ?](#4)   
    A. [A. Incohérence statistique](#4.A)   
    B. [B. Le temps d’attente](#4.B)   
    C. [C. Une question de consentement](#4.C)   
    D. [D. Des "bugs" de l'outil numérique SI-DEP ont pu affecter la collecte des données brutes de dépistage](#4.D)   
    E. [E. Dépendance à la coopération des laboratoires](#4.E)   
5) [5) Quelle réalités ces données/indicateurs sont-ils censés représenter ?](#5)   
